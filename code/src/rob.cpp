...
bool rob_check_space(ROB *rob)
{
    ...
    /*
     * our simulator is weird
     * so the tail pointer points to the next available spot!!!
     * if tail pointer entry is invalid, then the ROB has space
     * if tail pointer entry is valid, that means its also the head pointer, so the ROB is full
     * basically, if the tail pointer is valid, then the ROB is full
     * */
    if (!rob->entries[rob->tail_ptr].valid)
    {
        return true;
    }
    return false;

//    return (rob->tail_ptr + 1) % (int)NUM_ROB_ENTRIES != rob->head_ptr;
}
...
int rob_insert(ROB *rob, InstInfo inst)
{
    ...
    if (rob_check_space(rob))
    {
        ...
        rob->entries[index].valid = true;
        rob->entries[index].exec = false;
        rob->entries[index].ready = false;
        rob->entries[index].inst = inst;

        /* move the tail_ptr to the next available spot */
        rob->tail_ptr = (rob->tail_ptr + 1) % (int) NUM_ROB_ENTRIES;
...
void rob_mark_exec(ROB *rob, InstInfo inst)
{
    ...
    /* the condition should be to not be <= tail_ptr, rather != tail_ptr as we have a circular buffer */
//    for (int i = rob->head_ptr; i != rob->tail_ptr; i = (i + 1) % (int)NUM_ROB_ENTRIES)
//    {
...
//    for (int i = rob->head_ptr; i != rob->tail_ptr; i = (i + 1) % NUM_ROB_ENTRIES)
//    {
//        if (rob->entries[i].inst.inst_num == inst.inst_num)
//        {
//            /* for extra security */
...
/* added a new function to get the next free ROB index */
int rob_get_next_free_entry(ROB *rob)
{
    if (rob_check_space(rob))
    {
        return rob->tail_ptr;
