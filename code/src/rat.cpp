...
//    printf("No segemntation fault upto here\n");
//    // print the current state of the RAT
//    rat_print_state(rat);

    /* Check if arf_id is within valid bounds */
    if (arf_id < 0 || arf_id >= MAX_ARF_REGS)
    {
        fprintf(stderr, "Error: arf_id %d is out of bounds\n", arf_id);
        return -1;
    }

    /* return prf_id of the valid entry */
...
//    rat->entries[arf_id].prf_id = 0; // todo: this should be -1, prev it was 0, maybe its doesn't matter as we don't check any invalid values
}
